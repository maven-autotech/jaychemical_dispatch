                    ...
                    if rec.picking_id == picking_obj1:
                        ...
                        if rec.state == "assigned":
                            ...
                            if rec.product_id.id == int(self.pro_id):
                               ...
                               if ser_lot:
                                   error = "this serial number already assigned"
                                   return http.request.render('scan_invert.inventory_in_page', {'track_pro': True,'error': error,'table':self.table, 'assigned_list':self.assigned_list.strip("  |  ")})
                               else:
                                   comp = request.env.user.company_id.id
                                   if comp == 1:
                                   ...
                                   return http.request.render('scan_invert.inventory_in_page',{'track_pro': True,'table':self.table, 'assigned_list':self.assigned_list.strip("  |  ")})
