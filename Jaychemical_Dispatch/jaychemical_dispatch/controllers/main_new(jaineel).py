...
class Inventory(http.Controller):
    ...
    def assign_serailnum(self, **kw):
        try:
            ...
            move_obj = request.env['stock.move'].sudo().search([('id','=',user_picking.move_id)])
            self.pro_id = move_obj.product_id.id
            # print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@",self.pro_id)
            id = kw['product'][:2]
            global pro
            pro = self.pro_id
            #self.pick_id = int(kw['picking_id'])
            self.pick_id = move_obj.picking_id.id
            # print("KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK",self.pro_id,id,self.pick_id)
            pro_track = request.env['product.product'].sudo().search([('id', '=', self.pro_id)])
            # print("LLLLLLLLLLLLLLLLLLLL", pro_track)
            comp = request.env.user.company_id.id
            self.stock_move_id = user_picking.move_id
            ...
            if self.track == "lot":
                ...
                return http.request.render('stock_transfer_jaychemical.inventory_in_page',
                                           {'pro': self.pro_id, 'track_pro': False,'table':self.table, 'not_assigned':not_assigned, 'assigned_list':self.assigned_list.strip("  |  ")})
            else:
                # print("JJJJJJJJJJJJJJJJJJ",not_assigned)
                return http.request.render('stock_transfer_jaychemical.inventory_in_page',
                                           {'pro': self.pro_id, 'track_pro': True,'table':self.table, 'not_assigned':not_assigned, 'assigned_list':self.assigned_list.strip("  |  ")})
        except:
            picking_obj = request.env['stock.move'].sudo().search([('product_id.id', '=', self.pro_id)])
            comp = request.env.user.company_id.id
            picking_obj1 = request.env['stock.picking'].sudo().search([('id', '=', self.pick_id),('company_id', '=', comp)])
    ...
    def assign_serailnum_check(self, **kw):
        if kw:
            ...
            self.stock_move_id = user_picking.move_id
            self.track = pro_track.product_tmpl_id.tracking
            print("77777777777777777777777777777777777777777777777777777777777777", self.track)
                ...
                for rec in picking_obj:
                    ...
                    if rec.picking_id == picking_obj1:
                        ...
                        if rec.state == "assigned":
                            print(":::::::::::::::::::::::::::",rec.product_id,rec.product_id.id,self.pro_id)
                            if rec.product_id.id == int(self.pro_id):
                               ser_lot = request.env['stock.production.lot'].sudo().search([('name', '=',kw['serial'])])
                               if ser_lot:
                                   error = "this serial number already assigned"
                                   return http.request.render('stock_transfer_jaychemical.inventory_in_page', {'track_pro': True,'error': error,'table':self.table, 'assigned_list':self.assigned_list.strip("  |  ")})
                               else:
                                   comp = request.env.user.company_id.id
                                   if comp == 1:
                                       print("PPPPPPPPPPPPPPPPPPPPPP",ser_list)
                                       if kw['serial'] in ser_list:
                                           error = "this serial number already assigned"
                                           return http.request.render('stock_transfer_jaychemical.inventory_in_page',
                                                                      {'track_pro': True, 'error': error,'table':self.table, 'assigned_list':self.assigned_list.strip("  |  ")})
                                       else:
                                            ...
                                            for move_line in picking_obj1.move_ids_without_package:
                                                print("***************0000000000000000000000000000000000000000000000***********************",
                                                      move_line.product_id.name, self.assigned_list)
                                                if move_line.product_id.name == picking_obj.product_id.name:
                                                    self.table[move_line.product_id.name] = [move_line.product_uom_qty,
                                                                                             move_line.product_uom_qty - move_line.quantity_done]
                                                    print("********************************", self.table)
                                            ...
                                            if rec.product_uom_qty == rec.quantity_done:
                                                print("PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP",request.env.user.company_id,kw['serial'],picking_obj1)
                                                comp = request.env.user.company_id.id
                                                print("77777777777777777777777777777777777777777", comp)
                                                print("company 11111111111111111111111111111111111111111")
                                                import werkzeug.utils
                                       ...
                                       if kw['serial'] in ser_list:
                                           ...
                                           return http.request.render('stock_transfer_jaychemical.inventory_in_page',
                                            ...
                                            print("PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP", rec.product_uom_qty,
                                                  rec.quantity_done)
                                            if rec.product_uom_qty == rec.quantity_done:
                                                print("PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP", rec.product_uom_qty,rec.quantity_done)
                                                comp = request.env.user.company_id.id
                                                print("77777777777777777777777777777777777777777", comp)
                                                print("company 11111111111111111111111111111111111111111")
                                                import werkzeug.utils
                                   ...
                                   return http.request.render('stock_transfer_jaychemical.inventory_in_page',{'track_pro': True,'table':self.table, 'assigned_list':self.assigned_list.strip("  |  ")})
...
class ManualTransfer(http.Controller):
    ...
    def ManualTransfer1(self, **kw):
        ...
        if self.picking_type == 'Delivery':
            ...
            # ----------------------------- FOR Table (End) ------------------------------------------------------------------------------------
            print(")))))))))))))))))))))))))))))))))))))))",table)
            return http.request.render('stock_transfer_jaychemical.manual_location_page', {'qty_reqired': reserve_qty,
                                                                        'qty_done': done_qty,
                                                                        'table1': table})
        else:
            # ------- FOR Done Qty & Reserv Qty (Start) ------------
            ...
            return http.request.render('stock_transfer_jaychemical.manual_transfer_page', {'qty_reqired': reserve_qty,
                                                                        'qty_done': done_qty,
                                                                        'table2': table})
        ...
        # try:
            ...
            if self.picking_type == 'Delivery Orders':
                ...
                if not lot_obj:
                    ...
                    return http.request.render('stock_transfer_jaychemical.manual_transfer_page', {'error': "Invalid serial number.",
                ...
                if user_picking.lot_serial in scanned_list:
                    return http.request.render('stock_transfer_jaychemical.manual_transfer_page', {'error1': "Already Scanned",
                                                                                'qty_reqired': reserve_qty,
                                                                                'qty_done': done_qty,
                                                                                'table1': table})
                ...
                #     return http.request.render('stock_transfer_jaychemical.fifo_warning_page', {'serial': kw['serial']})
                # else:
                fifo = []
                last_status = None
                    ...
                    for move_line in obj.move_line_ids_without_package:
                        if lot_obj.id in fifo:
                            ...
                            if move_line.product_id == product and move_line.qty_done < move_line.product_uom_qty:
                                ...
                                return http.request.render('stock_transfer_jaychemical.manual_transfer_page',
                ...
                if tmp_dq != tmp_rq:
                    print("------->>>>> ------->>>>>>",table)
                    return http.request.render('stock_transfer_jaychemical.manual_transfer_page', {'success': "Done",
                                                                                'qty_reqired': tmp_rq,
                                                                                'qty_done': tmp_dq,
                                                                                'table1': table})
                else:
                    obj.button_validate()
                    return http.request.render('stock_transfer_jaychemical.complete_page', {})
            else:
                pass
                    ...
                    for obj in trf_obj:
                        for move_line in obj.move_line_ids_without_package:
                            if move_line.lot_id.name == kw['serial']:
                                ...
                                pass
                    print("-------------------------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                    return http.request.render('stock_transfer_jaychemical.manual_location_page', {'tracking': self.tracking})
                else:
                    return http.request.render('stock_transfer_jaychemical.manual_transfer_page', {'error': "Invalid lot/Serial number.",
                                                                                'qty_reqired': reserve_qty,
                                                                                'qty_done': done_qty,
                                                                                'table2': table})
            else:
    ...
    def Location(self, **kw):
        try:
            ...
            if loc_obj:
                ...
                return http.request.render('stock_transfer_jaychemical.manual_location_page', {'error': "Invalid Location."})
        except:
            import werkzeug.utils
            action_id = request.env.ref('stock.stock_picking_type_action')
            return werkzeug.utils.redirect('/web#action={}'.format(action_id.id))
